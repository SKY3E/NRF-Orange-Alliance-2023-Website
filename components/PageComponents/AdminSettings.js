          ...
          {userRequests.map((user) => (
            <div className="flex flex-col xl:flex-row" key={user}>
              <form onSubmit={(event) => handleAllowAccess(event, user)}>
                <p name="user" className="bg-white rounded mb-2 text-black text-center leading-8 px-2 mr-2">{user}</p>
                <button type='submit' className="bg-green-600 hover:bg-opacity-50 rounded h-8 w-56">Allow Access</button>
              </form>
            </div>
          ))}
        </div>
