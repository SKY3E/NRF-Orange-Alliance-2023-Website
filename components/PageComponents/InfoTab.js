...
function ShowTabButton({ showTab, onTabButtonClick }) {
  if (showTab === "RecentEvents") {
    return (
      <section>
        <button value="Admin" onClick={onTabButtonClick} className="bg-blue-900 bg-opacity-50 hover:bg-gray-700 hover:bg-opacity-50 rounded h-6 w-36 text-white">Admin</button>
      </section>
    );
  } else if (showTab === "Admin") {
    return (
      <section>
        <button value="RecentEvents" onClick={onTabButtonClick} className="bg-blue-900 bg-opacity-50 hover:bg-gray-700 hover:bg-opacity-50 rounded h-6 w-36 text-white">Recent Events</button>
      </section>
    );
  }
}
