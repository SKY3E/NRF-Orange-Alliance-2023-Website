...
import { getEventWithKey, getTeamsWithEvent, getRankingsWithEvent, getMatchesWithEvent } from '@/lib/orangealliance';
      ...
      <div className="flex flex-col mr-4">
        ...
        <article className="rounded bg-blue-900 bg-opacity-50 p-2 mr-4 mb-2 text-white">
          <div className="flex items-center">
            ...
            {showTeams ? (
              <button onClick={() => changeTeamView()} className="bg-green-600 hover:bg-opacity-50 rounded h-8 px-4 mr-2 ml-auto">Hide</button>
            ) : (
              <button onClick={() => changeTeamView()} className="bg-green-600 hover:bg-opacity-50 rounded h-8 px-4 mr-2 ml-auto">Show</button>
            )}
          </div>
          {showTeams ? (
            <div>
        ...
        <article className="rounded bg-blue-900 bg-opacity-50 p-2 mr-4 mb-2 text-white">
          <div className="flex items-center">
            ...
            {showMatches ? (
              <button onClick={() => changeMatchView()} className="bg-green-600 hover:bg-opacity-50 rounded h-8 px-4 mr-2 ml-auto">Hide</button>
            ) : (
              <button onClick={() => changeMatchView()} className="bg-green-600 hover:bg-opacity-50 rounded h-8 px-4 mr-2 ml-auto">Show</button>
            )}
          </div>
          {showMatches ? (
            <div>
              ...
              {eventMatches.length > 0 ? (
                eventMatches.map((match) => (
                  <div className="flex" key={match.matchName}>
                    <button onClick={() => console.log(match.matchName)} className="bg-green-600 hover:bg-opacity-50 rounded mb-2 h-8 px-4 mr-2">View</button>
                    <p className="bg-white rounded text-black text-center leading-8 px-2 mb-2 w-36 mr-2">{match.matchName}</p>
                  </div>
                ))
              ) : (
                <p className="bg-white rounded text-black text-center leading-8 px-2">No matches found.</p>
              )}
            </div>
          ) : null}
        </article>
        ...
        <div className="flex items-center">
            ...
            {showRankings ? (
              <button onClick={() => changeRankingView()} className="bg-green-600 hover:bg-opacity-50 rounded h-8 px-4 mr-2 ml-auto">Hide</button>
            ) : (
              <button onClick={() => changeRankingView()} className="bg-green-600 hover:bg-opacity-50 rounded h-8 px-4 mr-2 ml-auto">Show</button>
            )}
          </div>
          {showRankings ? (
            <div>
