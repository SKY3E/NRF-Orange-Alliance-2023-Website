...
export async function getRecentBestTeams() {
  ...
  const start = new Date(today.getTime() - 100 * 24 * 60 * 60 * 1000).toISOString();
...
export async function getTeamsWithKey(teamNumber) {
  ...
  const teamsWithKey = allTeams.filter(team => team.teamKey.includes(teamNumber.toString()));
  console.log(teamsWithKey);
  return teamsWithKey;
}
...
async function getEventData(eventKey) {
  // Retrieve the event data with the desired keys
  // For example, using the orange alliance npm package:
  const event = await toa.getEvent(eventKey);
  if (event) {
    return event;
  }
